...
describe('SWR Sync Storage', () => {
  ...
  describe(syncWithStorage, () => {
    test('should sync existing localStorage keys to the SWR cache', () => {
      localStorage.setItem('swr-key-1', JSON.stringify({ key: 1 }));
      localStorage.setItem('swr-key-2', JSON.stringify({ key: 2 }));
    ...
    test('should sync existing sessionStorage keys to the SWR cache', () => {
      sessionStorage.setItem('swr-key-1', JSON.stringify({ key: 1 }));
      sessionStorage.setItem('swr-key-2', JSON.stringify({ key: 2 }));
